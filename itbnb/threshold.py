...
class ThresholdOptimizer(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        self.metric_mats_ = {
            m: np.zeros((n_folds, n_tau), dtype=np.float32)
        ...
        for k, (train_idx, test_idx) in enumerate(self.folds_):
            ...
            estimator_params = (
                {} if self.estimator_params is None else self.estimator_params
            )
            try:
                model = self.estimator_class(**estimator_params)
            except TypeError as e:
                raise TypeError(
                    f"Invalid estimator_params passed to estimator_class: {e}"
                )
    ...
    def _compute_taus(self):
        ...
        for m in self.available_metrics:
            ...
            maximize = m not in ["fnr", "fpr", "misclassification_error"]
            self.best_tau_[m] = self._best(mat, maximize=maximize)
        fpr_mat = self.metric_mats_["fpr"]
        fnr_mat = self.metric_mats_["fnr"]
        self.best_tau_["balanced_error"] = self.tau_grid_[
            np.argmin(np.mean(fpr_mat + fnr_mat, axis=0))
        ]
        return self
